...
import { Deck, StudySession, Word } from '@/types';
...
export async function syncNow(uid: string, since: string | null): Promise<SyncResult> {
  ...
  // 2. 원격 변경분 받아서 병합
  const [remoteDecks, remoteWords, remoteSessions] = await Promise.all([
  ...
  return {
    pulled: remoteDecks.length + remoteWords.length + remoteSessions.length,
    pushed: mergedDecks.length + mergedWords.length + mergedSessions.length,
