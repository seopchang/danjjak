...
- ⚠️ **알림·애니메이션 실기기 미검증.** 2026-08-03 2차 세션에서 추가한 기능들(아래 8장).

## 4-2. 로그인 오류 조사 (2026-08-03 2차 세션, 미해결)

사용자가 APK로 로그인 시도 → **"네트워크에 연결할 수 없습니다"** 표시됨.
이 문구는 `auth/network-request-failed` 코드에서만 나온다(`auth-store.ts`의 `toKoreanMessage`).

**배제된 원인** (직접 확인함):
- ✅ GitHub Secrets 6개 모두 존재 → APK에 Firebase 설정 들어감.
  (설정이 비었으면 설정 화면에 "동기화가 설정되지 않았습니다"가 떠야 하는데 로그인 폼이 떴음)
- ✅ `AndroidManifest.xml`에 `android.permission.INTERNET` 있음 (로컬 prebuild로 확인).
- ✅ Firebase 콘솔 → Authentication → 로그인 방법 → **이메일/비밀번호 "사용 설정됨"** (사용자가 스크린샷으로 확인).

**아직 안 해본 것**:
- 실기기 로그를 못 봄. 에러 메시지가 한국어로 뭉개져서 원본 코드/메시지를 알 수 없다.
  → 진단하려면 `toKoreanMessage`에서 원본 코드를 함께 노출하거나, 설정 화면에 진단 버튼을 두는 게 빠르다.
- 웹(`npx expo start --web`)에서 같은 계정으로 로그인해보기. 되면 RN 환경 문제, 안 되면 Firebase 설정 문제로 갈린다.
  **단 로컬 `.env`가 필요하다**(Firebase 콘솔 → 프로젝트 설정 → 내 앱 → SDK 설정에서 복사).
- 폰의 VPN/사설 DNS/시간 설정 확인 (SSL 검증 실패도 network-request-failed로 나타난다).
...
## 8. 2026-08-03 2차 세션에서 추가한 것

사용자 요청 4건을 반영했다. `npx tsc --noEmit` 통과, 전 라우트 Metro 번들 통과,
`@firebase/auth/dist/rn` 단독 해석 확인까지 마쳤다. **실기기 확인은 아직 안 했다.**

| 요청 | 반영 내용 |
|---|---|
| 예문 빼기 | `Word`에서 `example` 필드 제거. 등록 폼·수정 폼·학습 화면·목록에서 모두 삭제 |
| 카드 모션 | 답변하면 카드가 옆으로 밀려나고 다음 카드가 반대쪽에서 들어옴. 뜻은 아래에서 올라오며 나타남 |
| 매일 복습 알림 | 매일 **00:00**에 로컬 알림. 설정 화면 스위치로 켜고 끔 |
| 레이아웃 | 홈에 "오늘의 복습" 큰 카드 + 전체/암기완료/미암기 타일. 단어장 추가는 + 버튼으로 접어둠 |

### 새로 생긴 파일
- `src/utils/review-queue.ts` — 복습 대상 선정. **미암기 우선 → 본 지 오래된 순 → 등록 순**, 최대 50개(`DAILY_REVIEW_LIMIT`). 덱을 가리지 않고 전체에서 뽑는다.
- `src/app/review.tsx` — 매일 복습 화면. 여러 덱이 섞이므로 끝날 때 **덱별로 나눠** `StudySession`을 기록한다(통계 화면이 덱 단위로 읽기 때문).
- `src/lib/notifications.ts` — 알림 권한/채널/스케줄. 같은 알림이 쌓이지 않게 예약 전에 지우고 다시 건다.
- `src/hooks/use-flash-card-motion.ts` — 카드 전환 모션. 학습 화면과 복습 화면이 공유한다.
- `src/stores/settings-store.ts` — 알림 on/off. **동기화 대상이 아니다**(기기마다 따로 설정).

### 걸려 넘어졌던 것
- **Reanimated 4에서 `runOnJS`는 deprecated**다. `react-native-worklets`의 **`scheduleOnRN`**을 써야 한다.
  호출 방식도 다르다: `runOnJS(fn)(a, b)` → `scheduleOnRN(fn, a, b)`.
- **IDE 진단이 편집 직후 한 박자 늦게 온다.** 이미 고친 줄에 대해 오류를 계속 보고했다.
  판단은 `npx tsc --noEmit` 결과로 할 것.
- `expo-notifications`는 네이티브 모듈이라 **알림은 APK에서만 제대로 확인된다.**
...
- **자정(00:00) 알림이 실제로 원하는 동작인지** 사용자와 재확인 필요. 자는 시간이라 아침으로 옮기고 싶을 수 있다. 시각은 `src/lib/notifications.ts`의 `REVIEW_HOUR`/`REVIEW_MINUTE` 두 줄만 고치면 된다.
